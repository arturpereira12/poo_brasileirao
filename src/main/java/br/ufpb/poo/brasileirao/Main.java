...
public class Main {
    ...
    /**
     * Seleciona times com base na forma (seleção ponderada)
     */
    private static List<Team> selectTeamsBasedOnForm(List<Team> allTeams, int count) {
        ...
        for (Team team : allTeams) {
            Integer form = team.getForm();
            int weightValue = (form != null) ? form : 50; // Valor padrão de 50 se não tiver forma
        ...
        while (selectedTeams.size() < count) {
            ...
            for (int i = 0; i < allTeams.size(); i++) {
                ...
                Integer form = team.getForm();
                int weightValue = (form != null) ? form : 50;
